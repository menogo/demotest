    ...
    value() {
      ...
      if (this.multi) {
        ...
        if (this.isUserSelect) {
          ...
          month = value[0][1] - 1
          const day = value[0][2]
          return this.render(year, month, '_WATCHRENDERVALUE_', [
            year,
            month,
            day
          ])
  ...
  methods: {
      ...
      if (this.value.length || this.multi) {
        if (this.range) {
          ...
          if (this.firstRender) {
            ...
            this.month =
              parseInt(handleMultiDay[handleMultiDay.length - 1][1], 10) - 1
            this.year = parseInt(
              handleMultiDay[handleMultiDay.length - 1][0],
              10
            )
            this.handleMultiDay = []
          } else {
    ...
    renderOption(year, month, i, playload) {
      ...
      if (this.range) {
        ...
        const monthLastDay = `${year}-${month + 1}-${new Date(
          year,
          month + 1,
          0
        ).getDate()}`
    ...
    render(y, m, renderer, payload) {
      ...
      if (
        ...
        for (
          let completionIndex = temp.length;
          completionIndex <= 5;
          completionIndex++
        ) {
