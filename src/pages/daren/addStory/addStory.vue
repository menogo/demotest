<template>
    <div class="page">
      ...
      <div class="btn-bg">
        <button class="save-btn" @click="goNext">下一步</button>
      </div>
    </div>
</template>
<script>
...
export default {
  data () {
    return {
      type: '',
      status: '',
      editorType: null, // 编辑页是否修改过 2：修改过
      content: null,
      contentText: null
    }
  },
  ...
  methods: {
    getTraTalentDetail() {
      this.$http.post(this.API.traPersonDetail).then(res => {
        if (res.code === 0) {
          ...
          if (this.status !== 0) {
            this.contentText = data.story
            this.onEditorReady()
          }
        }
      })
    },
    onEditorReady() { // 初始化富文本编辑器
      const that = this
      wx.createSelectorQuery().select('#editor').context(function(res) {
        that.editorCtx = res.context
        if (that.editorType === 2) {
          that.editorCtx.setContents({ // 修改历史数据
            html: that.content
          })
        } else {
          that.editorCtx.setContents({ // 首次编辑或有历史数据
            html: that.contentText,
            success: (res) => {
              console.log(res)
            },
            fail: (res) => {
    ...
    goNext () {
      if (this.content) { // 修改过
        wx.setStorageSync('editStory', this.content.html)
      } else { // 未修改
        wx.setStorageSync('editStory', this.contentText)
      }
      wx.navigateBack()
    }
  }
