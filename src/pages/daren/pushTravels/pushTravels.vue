<template>
    <div class="page">
      <div class="push-box">
        <input type="text" class="title-input" placeholder="游记标题" v-model="title">
        <div class="input-item">
          <input type="text" placeholder="目的地"  v-model="destination" @focus="inputFocus($event)">
          <!-- <input type="text" maxlength = "11" placeholder="出行时间"  v-model="travelTime"> -->
          <div  class="dis-ine">
            <picker mode="date"
              :value="travelTime"
              end="2217-09-01" @change="bindDateChangeStart($event)" style="position:reletive;">
              <input type="text"  placeholder="出行时间"  v-model="travelTime" disabled>
              </picker>
          </div>
          <input type="number" maxlength = "5" class="mr0" placeholder="出行人数"  v-model="travelNum" @focus="inputFocus($event)">
          <input type="number" maxlength = "5" placeholder="人均消费"  v-model="price"  @focus="inputFocus($event)">
          <input type="number" maxlength = "4" placeholder="出行天数" v-model="travelDay">
        </div>
        <!-- <textarea name="" id="" cols="30" rows="10" class="main-input" placeholder="游记正文"></textarea> -->
        <!-- <editor id="editor" placeholder="请输入文本" @ready="onEditorReady" @input="bindInput" @statuschange="onStatusChange"></editor> -->
        
        <div class="editor-box">
          <editor
            id="editor"
            class="ql-container"
            placeholder="游记正文 "
            showImgSize
            showImgToolbar
            showImgResize
            @focus='onbindfocus'
            @blur='onbindblur'
            @statuschange="onStatusChange"
            @input="inputEditor"
          ></editor>
          <div class="toolbar"  @click="format">
            <i class="iconfont icon-charutupian" @click="insertImage"></i>
            <!-- <img :src="addImg" alt=""  @click="insertImage"> -->
          </div>
        </div>
      </div>
      <div class="btn-bg">
        <button class="save-btn" @click="goNext">发布游记</button>
      </div>
    </div>
</template>
<script>
import Http from '../../../utils/http'
export default {
  data () {
    return {
      title: '',
      startTime: '',
      destination: '',
      travelTime: '',
      travelNum: '',
      price: '',
      travelDay: '',
      formats: {},
      content: null,
      // showAddImg: false,
      placeholder: '阿斯利康大家发卡拉三等奖分开了',
      addImg: require('../../../../static/images/addImg.png'),
      themeImg: require('../../../../static/images/common/themeBg.jpg'),
      miniIcon2: require('../../../../static/images/common/miniIcon2.jpg')
    }
  },
  onLoad() {
    this.title = ''
    this.destination = ''
    this.travelTime = ''
    this.travelNum = ''
    this.price = ''
    this.travelDay = ''
  },
  onShow () {
    this.onEditorReady()
  },
  methods: {
    inputFocus (e) {
      console.log(e.mp.detail.height)
    },
    bindDateChangeStart (res) {
      this.travelTime = res.mp.detail.value
    },
    inputEditor(e) {
      this.content = e.mp.detail
      console.log('content', this.content)
    },
    onbindfocus(event) {
      console.log('111')
      // this.showAddImg = true
    },
    onbindblur(event) {
      // this.showAddImg = false
    },
    onEditorReady() {
      const that = this
      wx
        .createSelectorQuery()
        .select('#editor')
        .context(function(res) {
          that.editorCtx = res.context
        })
        .exec()
    },
    format(e) {
      console.log(e)
      let { name, value } = e.target.dataset
      if (!name) return
      console.log('format', name, value)
      this.editorCtx.format(name, value)
    },
    insertImage() {
      const that = this
      wx.chooseImage({
        count: 1,
        success: function (res) {
          console.log('res.tempFilePaths[0]', res.tempFilePaths[0])
          wx.showLoading()
          wx.uploadFile({
            url: Http.baseURL + '/uploadByUrl',
            filePath: res.tempFilePaths[0],
            name: 'file',
            success: function (r) {
              r.data = JSON.parse(r.data)
              if (r.data.code === 0) {
                that.editorCtx.insertImage({
                  // src: res.tempFilePaths[0],
                  src: r.data.result,
                  // width: '80%',
                  success: function () {
                    console.log('insert image success')
                  }
                })
              } else {
                wx.showToast({
                  title: '上传失败',
                  icon: 'none'
                })
              }
            },
            complete() {
              wx.hideLoading()
            }
          })
          // that.editorCtx.insertImage({
          //   src: res.tempFilePaths[0],
          //   width: '80%',
          //   success: function () {
          //     console.log('insert image success')
          //   }
          // })
        }
      })
    },
    goNext () {
      this.$http.post(this.API.saveTravelNote, {
        data: {
          title: this.title,
          destination: this.destination,
          travelTime: this.travelTime,
          travelNum: this.travelNum,
          price: this.price,
          travelDay: this.travelDay,
          content: this.content.html
        }
      }).then(res => {
        if (res.code === 0) {
          wx.showToast({
            title: '发布成功',
            icon: 'none'
          })
          setTimeout(function () {
            wx.switchTab({
              url: `/pages/me/main`
            })
          }, 1000)
        } else {
          wx.showToast({
            title: res.message,
            icon: 'none'
          })
        }
      })
    },
    showJson(style) {
      let s = []
      for (let i in style) {
        s.push(i + ':' + style[i])
      }
      s = s.join(';')
      console.log(s)
      return s
    }
  },
  computed: {
    computedClassObject () {
      return this.showJson({
        bottom: `${this.bottom}rpx`
      })
    }
  }
}
</script>
<style lang="scss" scoped>
// .back-page {
//     position: fixed;
//     top: 0;
//     left: 0;
//     right: 0;
//     bottom: 0;
//     z-index: 10000;
//     background: rgba(0,0,0,0.40);
// }

.release {
    display: flex;
    align-items: flex-end;
    justify-content: space-between;
    box-sizing: border-box;
    // position: absolute;
    // left: 0;
    // bottom: 0;
    width: 100%;
    padding: 20rpx 12rpx;
    background: #FFFFFF;
}

.release .text {
    background: #F7F7F7;
    border-radius: 8rpx;
    width: 612rpx;
    min-height: 40rpx;
    max-height: 120rpx;
    border-width: 10rpx 20rpx;
    border-style: solid;
    border-color: #F7F7F7;
    line-height: 40rpx;
    font-family: PingFangSC-Regular;
    font-size: 28rpx;
    color: #333333;
}

.release .submit {
    width: 138rpx;
    font-family: PingFangSC-Regular;
    font-size: 32rpx;
    color: #1B9AF4;
    text-align: center;
    height: 64rpx;
    line-height: 64rpx;
}

.input_null {
    margin-left: 12rpx;
    font-family: PingFangSC-Regular;
    font-size: 28rpx;
    color: #939393;
}

.page{
  height: 100vh;
  background: #fff;
}
.dis-ine{
  display: inline-block;
}
.push-box{
  background: #fff;
  padding: 30rpx;
  .title-input{
    border-bottom: 1rpx solid #666666;
    padding: 10rpx 0;
    font-size: 30rpx;
  }
  .input-item{
    input{
      font-size: 28rpx;
      width: 190rpx;
      padding: 10rpx 0;
      border-bottom: solid 1rpx #666666;
      margin-right: 60rpx;
      text-align: center;
      display: inline-block;
      margin-top: 60rpx;
    }
    .mr0{
      margin-right: 0
    }
  }
  .main-input{
    font-size: 28rpx;
    width: 690rpx;
    height: 720rpx;
    border: solid 1rpx #666666;
    box-sizing: border-box;
    padding: 20rpx;
    margin-top: 60rpx;
  }
}
.qrCode-icon{
  width: 32rpx;
  height: 34rpx;
  background: url(../../../../static/images/meIcon.png);
  background-size: 400rpx 160rpx;
  background-position: -269rpx -38rpx;
  float: right;
  margin-top: 16rpx;
  margin-right: 12rpx;
}
.btn-bg{
  background: #fff;
  position: fixed;
  bottom: 0;
  width: 100%;
  padding: 20rpx 0;
  .save-btn{
    background: #fbb03b;
    color: #fff;
    font-size: 28rpx;
    border: none;
    height: 80rpx;
    line-height: 80rpx;
    margin: 0 30rpx;
    border-radius: 50rpx;
    border-radius: 50rpx;
  }
}
.editor-box{
  position: relative;
  .ql-container {
    border: 1rpx solid #eee;
    padding: 70rpx 20rpx 50rpx 20rpx;
    margin-top: 30rpx;
    box-sizing: border-box;
    width: 100%;
    height: 660rpx;
    font-size: 28rpx;
    line-height: 1.5;
  }
  .ql-active {
    color: #06c;
  }
}
.toolbar{
  animation: mymove .3s forwards;
  padding: 10rpx 20rpx;
  position: absolute;
  top: 0;
  left: 0;
  color: #666666;
  box-sizing: border-box;
  img{
    width: 50rpx;
    height: 50rpx;
    transform:scale(1.3);
  }
  i{
    display: inline-block;
    margin-right: 20rpx;
    width:8%;
  }
  .iconfont {
    font-family: "iconfont" !important;
    font-size: 40rpx;
    font-style: normal;
    -webkit-font-smoothing: antialiased;
    -moz-osx-font-smoothing: grayscale;
  }
  .icon-charutupian:before {
    content: "\ec82";
  }
  .icon-zitijiacu:before {
    content: "\ec8a";
  }
  .icon-zitixiahuaxian:before {
    content: "\ec8e";
  }
  .icon-format-header-2:before {
    content: "\e75c";
  }
  .icon-format-header-3:before {
    content: "\e75d";
  }
  .icon-zitixieti:before {
    content: "\ec8f";
  }
  .icon-youxupailie:before {
    content: "\ec86";
  }
  .icon-wuxupailie:before {
    content: "\ec83";
  }
}
@keyframes mymove
{
  from {opacity: 0 }
  to {opacity:1;}
}
@-webkit-keyframes mymove
{
  from {opacity: 0 }
  to {opacity:1;}
}
@font-face {
  font-family: "iconfont";
  src: url("//at.alicdn.com/t/font_945958_zfsfjju1dim.eot?t=1547618146468"); /* IE9 */
  src: url("//at.alicdn.com/t/font_945958_zfsfjju1dim.eot?t=1547618146468#iefix")
      format("embedded-opentype"),
    /* IE6-IE8 */
      url("data:application/x-font-woff2;charset=utf-8;base64,")
      format("woff2"),
    url("//at.alicdn.com/t/font_945958_zfsfjju1dim.woff?t=1547618146468")
      format("woff"),
    url("//at.alicdn.com/t/font_945958_zfsfjju1dim.ttf?t=1547618146468")
      format("truetype"),
    /* chrome, firefox, opera, Safari, Android, iOS 4.2+ */
      url("//at.alicdn.com/t/font_945958_zfsfjju1dim.svg?t=1547618146468#iconfont")
      format("svg"); /* iOS 4.1- */
}
</style>